...
class DAT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, num_classes=1000, expansion=4,
        ...
        if self.conditional:
            # TODO parametrize 1000
            self.class_proj = nn.Linear(dim_stem + 1000, dim_stem)
    ...
    def forward(self, x):
        ...
        # torch.Size([64, 3, 224, 224]) x
        # torch.Size([64, 1000]) query
        # torch.Size([64, 96, 56, 56]) proj

        x = self.patch_proj(x)

        if query_vector is not None:
            x = x.transpose(0, 2, 3, 1)  # 64, 56, 56, 96
            query_vector = query_vector.unsqueeze(1).unsqeeze(2)  # 64, 1, 1, 1000
            x = torch.cat((x, query_vector), dim=-1)  # 64, 56, 56, 1096
            x = self.class_proj(x)  # 64, 56, 56, 96
            x = x.transpose(0, 3, 1, 2)  # 64, 96, 56, 56
